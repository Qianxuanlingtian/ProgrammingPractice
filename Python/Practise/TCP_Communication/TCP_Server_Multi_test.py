...
def dispose_client_request(tcp_client_1,tcp_client_address):
    ...
    while True:
        ...
        if recv_data:
            ...
            print("客户端发来的消息是:", recv_data)
            if recv_data == b'\xff\x01\x06\x08\x55':
                send_data = b'\xff\x07\x07\x08\x55'
            elif recv_data == b'\xff\x01\x06\x09\x55':
                send_data = b'\xff\x07\x07\x09\x55'
